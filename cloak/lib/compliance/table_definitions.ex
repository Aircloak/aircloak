defmodule Compliance.TableDefinitions do
  ...
  def plain(produce_collections_rather_than_tables), do:
  ...
  def encoded(produce_collections_rather_than_tables), do:
    raw_table_definitions()
    |> prepare_as_collections(produce_collections_rather_than_tables)
    |> Enum.map(fn({table, %{columns: raw_columns} = definitions}) ->
  ...
  defp prepare_as_collections(collections, true) do
    {primary_collections, sub_collections} = Enum.split_with(collections, fn({_name, definition}) ->
    ...
    Enum.reduce(sub_collections, primary_collections, fn({_name, sub_collection}, primaries_acc) ->
      ...
      primary_columns = Enum.reduce(sub_collection.columns, primary_collection.columns, fn({name, definition}, acc) ->
    ...
    %{
      ...
      addresses: %{
        columns: %{
          ...
          'home.city': %{type: :text, decoders: [:base64, aes_cbc_128: [key: Data.encryption_key()]]},
          'work.city': %{type: :text, decoders: [:base64, aes_cbc_128: [key: Data.encryption_key()]]},
      ...
      notes_changes: %{
        ...
        columns: %{
          ...
          title: %{type: :text, decoders: [:base64, aes_cbc_128: [key: Data.encryption_key()]]},
          content: %{type: :text, decoders: [:base64, aes_cbc_128: [key: Data.encryption_key()]]},
          'changes.change': %{type: :text, decoders: [:base64, aes_cbc_128: [key: Data.encryption_key()]]},
