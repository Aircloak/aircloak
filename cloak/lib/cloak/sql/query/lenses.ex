defmodule Cloak.Sql.Query.Lenses do
  ...
  deflens(
    subquery_noise_layers(),
    do: direct_subqueries() |> Lens.key(:ast) |> Lens.key(:noise_layers) |> Lens.all()
  )

  @doc "Lens focusing on all queries (subqueries and top-level) of a query."
  deflens(
    all_queries(),
    do: Lens.both(Lens.recur(direct_subqueries() |> Lens.key(:ast)), Lens.root())
  )
  ...
  deflens(join_conditions_terminals(), do: join_conditions() |> conditions_terminals())
