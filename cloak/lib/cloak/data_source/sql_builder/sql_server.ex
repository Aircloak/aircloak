defmodule Cloak.DataSource.SqlBuilder.SQLServer do
  ...
  def function_sql("hex", [arg]),
    do: ["LOWER(CONVERT(nvarchar, CAST(", arg, " AS varbinary), 2))"]
  ...
  def function_sql("substring", [arg1, arg2]),
    do: ["SUBSTRING(", arg1, ", ", arg2, ", LEN(", arg1, "))"]
  ...
  def limit_sql(limit, offset),
    do: [" OFFSET ", to_string(offset), " ROWS FETCH NEXT ", to_string(limit), " ROWS ONLY"]
  ...
  @impl Dialect
  def order_by(column, :asc, :nulls_last),
    do: ["CASE WHEN ", column, " IS NULL THEN 1 ELSE 0 END, ", column, " ASC"]

  def order_by(column, :desc, :nulls_first),
    do: ["CASE WHEN ", column, " IS NULL THEN 0 ELSE 1 END, ", column, " DESC"]
