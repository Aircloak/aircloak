defmodule Cloak.DataSource.MongoDB.Projector do
  ...
  def map_column(%Column{aggregate?: true, db_function?: true, function: fun, function_args: [arg], alias: alias}), do:
  ...
  defp begin_parse_column(%Column{db_function?: true, function: fun} = column) when fun != nil do
    non_null_args =
      column
      |> extract_fields()
      |> Enum.map(&%{'$gt': ["$" <> &1, nil]})
  ...
  defp extract_fields(%Column{db_function?: true, function: fun, function_args: args}) when fun != nil, do:
  ...
  defp parse_column(%Column{db_function?: true, function: "length", function_args: [%Column{name: name}]})
    when is_binary(name), do: "$" <> name <> ".length"
  defp parse_column(%Column{db_function?: true, function: {:cast, type}, function_args: [value]}), do:
    parse_function("cast", [parse_column(value), value.type, type])
  defp parse_column(%Column{db_function?: true, function: fun, function_args: [arg]}) when fun != nil, do:
    parse_function(fun, parse_column(arg))
  defp parse_column(%Column{db_function?: true, function: fun, function_args: args}) when fun != nil, do:
