defmodule Cloak.SapHanaHelpers do
  ...
  def ensure_schema!(conn_or_connection_params, schema_name),
    do: :ok = ensure_schema(conn_or_connection_params, schema_name)
  ...
  def insert_rows!(conn, schema_name, table_name, columns, rows) do
    ...
    |> Enum.each(fn {sql, values} ->
      execute!(
        conn,
        ~s/INSERT INTO "#{schema_name}"."#{table_name}"(#{quoted_column_names}) (#{sql})/,
        values
      )
    end)
