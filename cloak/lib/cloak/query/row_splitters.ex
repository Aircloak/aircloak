defmodule Cloak.Query.RowSplitters do
  ...
  def split(rows, query), do: split_rows(rows, Query.outermost_selected_splitters(query))

  # -------------------------------------------------------------------
  # Internal functions
  # -------------------------------------------------------------------

  defp name_outermost_selected_splitters(query),
  ...
  defp name_outermost_splitters_in_select_list(query),
    do: update_in(query, [Lenses.outermost_selected_splitters()], &name_splitter/1)

  defp sync_references_to_selected_outermost_splitters(query),
    do:
      Enum.reduce(Query.outermost_selected_splitters(query), query, fn expression, query ->
        Query.replace_expression(query, %Expression{expression | name: nil}, expression)
      end)

  defp add_outermost_splitters_to_db_columns(query),
    do: Enum.reduce(Query.outermost_selected_splitters(query), query, &Query.add_db_column(&2, &1))

  defp name_splitter(splitter),
    do: %Expression{
      splitter
      | name:
          Enum.join(
            [
              "splitter",
              Function.readable_name(splitter.function),
  ...
  defp split_rows(rows, []),
    # optimization -> no row splitters, so we're just passing the input enumerable
    do: rows

  defp split_rows(rows, row_splitters), do: Stream.flat_map(rows, &split_row(&1, row_splitters))

  defp split_row(row, row_splitters),
    do:
      row_splitters
  ...
  defp splitters_values([splitter | remaining_splitters], row) do
    for splitter_value <- Expression.splitter_values(splitter, row),
        remaining_splitters_values <- splitters_values(remaining_splitters, row) do
