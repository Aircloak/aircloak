defmodule Cloak.AnalystTable.Compiler do
  ...
  @spec compile(String.t(), String.t(), Query.analyst_id(), Cloak.DataSource.t()) ::
          {:ok, Query.t()} | {:error, String.t()}
  def compile(table_name, statement, analyst, data_source) do
    with :ok <- supports_analyst_tables?(data_source),
         :ok <- verify_table_name(table_name, data_source),
         {:ok, query} <- compile_statement(statement, analyst, data_source),
  ...
  defp compile_statement(statement, analyst, data_source) do
    with {:ok, parsed_query} <- Cloak.Sql.Parser.parse(statement),
         {:ok, query} <- Cloak.Sql.Compiler.compile_direct(parsed_query, analyst, data_source),
