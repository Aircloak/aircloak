defmodule Compliance.Runtime do
  ...
  def handle_cast({:record, data_source, time}, state),
    do: {:noreply, Map.update(state, data_source, [time], &[time | &1])}
  ...
  defp output_stats(state) do
    {measurements, small_data_sources} =
      Enum.map(state, fn {data_source, measurements} ->
    ...
    if length(small_data_sources) > 0 do
      ...
      IO.puts(
        "The following data sources were ignored as they had less than #{@min_measurements} " <>
  ...
  defp render_graph(data) do
    ...
    graph =
      values
      |> Enum.reduce(:array.new(@graph_width), fn {index, value}, acc ->
        :array.set(index, value, acc)
      end)
      |> :array.to_list()
      |> Enum.join("")
  ...
  defp render_stats(data) do
    data_as_rows =
      Enum.map(
        ...
        &[
          &1[:title],
          &1[:number_measurements],
          &1[:min],
          &1[:fifth_percentile],
          &1[:median],
          &1[:ninty_fifth_percentile],
          &1[:ninty_nine_percentile],
          &1[:max]
        ]
      )
