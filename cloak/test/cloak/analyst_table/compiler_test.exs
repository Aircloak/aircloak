defmodule Cloak.AnalystTable.CompilerTest do
  ...
  describe "compiled sql - " do
    test "simple analyst table" do
      {:ok, query} = Compiler.compile("table_name", "select user_id, x from mv1", data_source())
    ...
    test "function support" do
      {:ok, query} = Compiler.compile("table_name", "select user_id, hash(y) from mv1", data_source())
    ...
    test "subquery support" do
      {:ok, query} = Compiler.compile("table_name", "select user_id, x from (select * from mv1) sq", data_source())
    ...
    test "tables are referenced by the user name" do
      {:ok, query} = Compiler.compile("table_name", "select user_id, x from mv2", data_source())
    ...
    test "tables can't be referenced by the db name" do
      assert {:error, error} = Compiler.compile("table_name", "select user_id, x from db_name_mv2", data_source())
      assert error == "Table `db_name_mv2` doesn't exist."
    end

    test "parsing error is reported" do
      assert {:error, error} = Compiler.compile("table_name", "select", data_source())
      assert error == "Expected `column definition` at line 1, column 7."
    end

    test "compilation error is reported" do
      assert {:error, error} = Compiler.compile("table_name", "select sqrt(y) from mv1", data_source())
    ...
    test "anonymized subqueries can't be materialized" do
      assert {:error, error} = Compiler.compile("table_name", "select x from mv1", data_source())
      assert error == "At least one user id column must be selected."
    end

    test "emulated subqueries can't be materialized" do
      assert {:error, error} = Compiler.compile("table_name", "select user_id, dec_b64(y) from mv1", data_source())
    ...
    test "analyst table can't be created if the driver doesn't support it" do
      ...
      assert {:error, error} = Compiler.compile("table_name", "select * from mv1", mongo_data_source)
    ...
    test "analyst table can't be created if the table with the same name already exists" do
      assert {:error, error} = Compiler.compile("mv1", "select user_id from mv1", data_source())
    ...
    test "analyst table can't be created if column names are duplicate" do
      assert {:error, error} = Compiler.compile("table_name", "select user_id, x, y, x, y, x from mv1", data_source())
      assert error == "Duplicate column names: `x`, `y`"

      assert {:error, error} =
               Compiler.compile("table_name", "select user_id, sqrt(x), sqrt(3) from mv1", data_source())
