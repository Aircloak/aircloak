defmodule Cloak.Sql.Range.Test do
  ...
  describe "find_ranges" do
    ...
    test ">/<= ranges in subquery having" do
      ...
      assert [
               %Range{
                 column: %{name: "avg", args: [%{name: "number"}]},
                 interval: {10, 20}
               }
    ...
    test "function range in SELECT" do
      query = compile("SELECT day(timestamp) FROM table")

      assert [%Range{column: %{name: "day"}, interval: :implicit}] = Range.find_ranges(query)
    end

    test "function range in expression in SELECT" do
      query = compile("SELECT day(timestamp) + 1 FROM table")
      assert [%Range{column: %{name: "+"}, interval: :implicit}] = Range.find_ranges(query)
    end

    test "function range on expression in SELECT" do
      query = compile("SELECT trunc(float + 1) FROM table")

      assert [%Range{column: %{name: "trunc"}, interval: :implicit}] = Range.find_ranges(query)
    end
    ...
    test "function ranges from subquery HAVING" do
      ...
      assert [%Range{column: %{name: "trunc"}, interval: :implicit}] = Range.find_ranges(subquery)
    end
    ...
    for function <- ~w(round trunc) do
      test "#{function} range in SELECT" do
        ...
        assert [%Range{column: %{name: unquote(function)}, interval: :implicit}] = Range.find_ranges(query)
      end
    end
    ...
    for part <- ~w(year quarter month day hour minute second) do
      test "date_trunc(#{part})" do
        ...
        assert [%Range{column: %{name: "date_trunc"}, interval: :implicit}] = Range.find_ranges(query)
      end
    end
  end
