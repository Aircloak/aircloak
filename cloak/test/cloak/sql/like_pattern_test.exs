defmodule Cloak.Sql.LikePattern.Test do
  ...
  describe "new" do
    ...
    test "switches the escape character to \\", do: assert({~S[a\\b\%~c], "\\"} = LikePattern.new(~S[a~\~b~%~~c], "~"))

    test "compresses multiple %%", do: assert({~S[%a%b%c%], "\\"} = LikePattern.new(~S[%%a%%%b%c%%%%], nil))
  ...
  describe "lowercase" do
    test "produces lowercase patterns for trivial like patterns",
      do: assert(LikePattern.new("abc", "\\") == LikePattern.new("AbC", "\\") |> LikePattern.lowercase())

    test "produces lowercase patterns for complex like patterns",
      do: assert(LikePattern.new("a%b_c", "\\") == LikePattern.new("A%b_C", "\\") |> LikePattern.lowercase())
  end
