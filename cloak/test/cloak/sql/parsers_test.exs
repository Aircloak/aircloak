defmodule Cloak.Sql.Parsers.Test do
  ...
  describe "sep_by1_eager" do
    ...
    test "multiple items",
      do: assert([["a", ",", "a", ",", "a"]] = Combine.parse("a,a,a", sep_by1_eager(char("a"), char(","))))
  ...
  describe "choice_deepest_error" do
    test "success - the first clause matches",
      do: assert(["a"] = Combine.parse("a", choice_deepest_error([char("a"), char("b") |> char("b")])))

    test "success - the second clause matches",
      do: assert(["b", "b"] = Combine.parse("bb", choice_deepest_error([char("a"), char("b") |> char("b")])))
  ...
  describe "return" do
    test "works outside of a pipeline", do: assert([:value] = Combine.parse("abc", return(:value)))

    test "works in a pipeline",
      do: assert([:value] = Combine.parse("abc", Combine.Parsers.Base.ignore(char("a")) |> return(:value)))
  end
end
