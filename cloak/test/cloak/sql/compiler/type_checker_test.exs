defmodule Cloak.Sql.Compiler.TypeChecker.Test do
  ...
  describe "IN" do
    ...
    test "forbids unclear IN lhs" do
      ...
      assert message =~
               ~r[Only `lower`, `upper`, .*, and `.*` can be used in the left-hand side of an IN operator]
    end
    ...
    test "allows clear IN lhs from subqueries",
      do:
        assert(
          {:ok, _, _} =
            compile(
              "SELECT COUNT(*) FROM (SELECT numeric AS number FROM table) x WHERE number IN (1, 2, 3)"
            )
        )
    ...
    test "forbids unclear IN lhs from subqueries" do
      assert {:error, message} =
               compile(
                 "SELECT COUNT(*) FROM (SELECT numeric + 1 AS number FROM table) x WHERE number IN (1, 2, 3)"
               )
    ...
    for function <- ~w(lower upper trim ltrim btrim extract_words) do
      test "allows #{function} in IN lhs" do
        assert {:ok, _, _} =
                 compile(
                   "SELECT #{unquote(function)}(string) AS x FROM table WHERE x IN ('a', 'b', 'c')"
                 )
      end
    end
    ...
    test "allows substring in IN lhs" do
      assert {:ok, _, _} =
               compile(
                 "SELECT SUBSTRING(string FROM 3) AS x FROM table WHERE x IN ('a', 'b', 'c')"
               )
    end
  end

  describe "negative conditions" do
    test "allows clear <> lhs",
      do: assert({:ok, _, _} = compile("SELECT COUNT(*) FROM table WHERE numeric <> 10"))
    ...
    test "forbids column <> unclear_column" do
      assert {error, {1, 44}} =
               error_with_location("SELECT COUNT(*) FROM table WHERE string <> upper(string)")
    ...
    test "forbids unclear_column <> column" do
      assert {error, {1, 34}} =
               error_with_location("SELECT COUNT(*) FROM table WHERE upper(string) <> string")
    ...
    test "allows clear NOT LIKE lhs",
      do:
        assert(
          {:ok, _, _} =
            compile("SELECT COUNT(*) FROM table WHERE string NOT LIKE '%some pattern_'")
        )
    ...
    test "allows clear NOT ILIKE lhs",
      do:
        assert(
          {:ok, _, _} =
            compile("SELECT COUNT(*) FROM table WHERE string NOT ILIKE '%some pattern_'")
        )
    ...
    test "forbids unclear NOT ILIKE lhs" do
      assert {:error, message} =
               compile(
                 "SELECT COUNT(*) FROM table WHERE upper(string) NOT ILIKE '%some pattern_'"
               )
    ...
    test "allows substring in <> lhs" do
      assert {:ok, _, _} =
               compile("SELECT SUBSTRING(string FROM 3) AS x FROM table WHERE x <> 'a'")
    end
  end
  ...
  describe "string-based conditions" do
    test "allows string manipulation functions on clear columns in positive conditions",
      do:
        assert(
          {:ok, _, _} = compile("SELECT COUNT(*) FROM table WHERE ltrim(string, 'abc') = 'foo'")
        )
    ...
    test "forbids string manipulation functions on unclear columns in top-level select",
      do:
        assert(
          {:error,
           "String manipulation functions cannot be combined with other transformations." <> _} =
    ...
    test "forbids complex expressions on the RHS of conditions with string manipulation functions",
      do:
        assert(
          {:error,
           "Results of string manipulation functions can only be compared to constants." <> _} =
    ...
    test "allows raw cast columns on the RHS of conditions with string manipulation functions",
      do:
        assert(
          {:ok, _, _} =
            compile(
              "SELECT COUNT(*) FROM table WHERE substring(string from 1) = cast(numeric as text)"
            )
        )
    ...
    test "allows string manipulation functions after a cast",
      do:
        assert(
          {:ok, _, _} =
            compile(
              "SELECT COUNT(*) FROM table WHERE btrim(cast(numeric as text), 'abc') = 'foo'"
            )
        )
    ...
    test "forbids string manipulation functions after nested casts",
      do:
        assert(
          {:error, _} =
            compile(
              "SELECT COUNT(*) FROM table WHERE btrim(cast(cast(numeric as real) as text), 'abc') = 'foo'"
            )
        )
    ...
    test "allows string-based functions after aggregator",
      do:
        assert(
          {:ok, _, _} =
            compile(
              "SELECT COUNT(*) FROM (SELECT uid FROM table GROUP BY uid HAVING left(max(string), 3) = 'foo') x"
            )
        )
    ...
    test "allows string-based functions before aggregator",
      do:
        assert(
          {:ok, _, _} =
            compile(
              "SELECT COUNT(*) FROM (SELECT uid FROM table GROUP BY uid HAVING max(left(string, 3)) = 'foo') x"
            )
        )
  end
  ...
  describe "ranges" do
    test "allows clear >=/< arguments",
      do:
        assert(
          {:ok, _, _} = compile("SELECT COUNT(*) FROM table WHERE numeric > 0 AND numeric < 10")
        )

    test "forbids unclear >=/< arguments" do
      assert {:error, narrative} =
               compile(
                 "SELECT COUNT(*) FROM table WHERE sqrt(numeric) > 0 AND sqrt(numeric) < 10"
               )
    ...
    test "allows casts in ranges",
      do:
        assert(
          {:ok, _, _} =
            compile("SELECT COUNT(*) FROM table WHERE CAST(string AS INTEGER) BETWEEN 0 AND 10")
        )
  end
  ...
  describe "exceptions" do
    for function <- ~w(upper lower ltrim btrim rtrim) do
      test "#{function} is allowed with IN" do
        assert {:ok, _, _} =
                 compile(
                   "SELECT COUNT(*) FROM table WHERE #{unquote(function)}(string) IN ('foo', 'bar', 'baz')"
                 )
      end
      ...
      test "#{function} is allowed with NOT IN" do
        assert {:ok, _, _} =
                 compile(
                   "SELECT COUNT(*) FROM table WHERE #{unquote(function)}(string) NOT IN ('foo', 'bar', 'baz')"
                 )
      end
    end
    ...
    test "substring is allowed with IN" do
      assert {:ok, _, _} =
               compile(
                 "SELECT COUNT(*) FROM table WHERE substring(string FROM 1 FOR 10) IN ('foo', 'bar', 'baz')"
               )
    end
    ...
    test "substring is allowed with NOT IN" do
      assert {:ok, _, _} =
               compile(
                 "SELECT COUNT(*) FROM table WHERE substring(string FROM 1 FOR 10) NOT IN ('foo', 'bar', 'baz')"
               )
    end
  end
