defmodule Cloak.Sql.TransformerTest do
  ...
  describe "group by uid" do
    test "select column" do
      assert flatten("""
             ...
             """) == transform("select col1 from table", &Transformer.group_by_uid/1)
    end
    ...
    test "select count(*)" do
      assert flatten("""
             ...
             """) == transform("select count(*) from table", &Transformer.group_by_uid/1)
    end
    ...
    test "select count(column)" do
      assert flatten("""
             ...
             """) == transform("select count(col1) from table", &Transformer.group_by_uid/1)
    end
    ...
    test "where filter" do
      assert flatten("""
             ...
             """) == transform("select count(*) from table where col1 = 0", &Transformer.group_by_uid/1)
    end
    ...
    test "group by" do
      assert flatten("""
             ...
             """) == transform("select col1 from table group by col1", &Transformer.group_by_uid/1)
    end
    ...
    test "group by with sum" do
      assert flatten("""
             ...
             GROUP BY uid_grouping.group_0
             """) == transform("select col1, sum(col2) from table group by col1", &Transformer.group_by_uid/1)
    end
    ...
    test "group by with having" do
      assert flatten("""
             ...
             """) == transform("select col1 from table group by col1 having sum(col2) = 0", &Transformer.group_by_uid/1)
    end
  ...
  describe "distinct statistics" do
    defp distinct_count_target(query) do
      [%{function: "count", function_args: [{:distinct, target}]} | _] = query.aggregators
      target
    end
