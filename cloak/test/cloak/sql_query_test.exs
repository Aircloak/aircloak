defmodule Cloak.SqlQueryTest do
  ...
  test "fully qualified table name" do
    assert %{select: ["foo"], from: "bar.baz"} == SqlQuery.parse!("select foo from bar.baz")
  end
  ...
  test "multiple fields" do
    assert %{select: ["foo", "bar"], from: "baz"} == SqlQuery.parse!("select foo, bar from baz")
  end

  test "whitespaces are ignored" do
    assert %{select: ["foo"], from: "baz"} == SqlQuery.parse!("select  foo\n from \n \n baz \n ; \n  ")
  end
  ...
  test "show tables" do
    assert %{show: :tables} == SqlQuery.parse!("show tables")
  end

  test "show columns" do
    assert %{show: :columns, from: "foo"} == SqlQuery.parse!("show columns from foo")
  end
