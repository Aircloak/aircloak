defmodule Cloak.DataSource.DecodersAndProjectionsTest do
  ...
  setup_all do
    ...
    tables = %{
      dec_proj_enc: %{user_id: "_id", db_name: "dec_proj_enc_base", decoders: [
        %{method: "text_to_integer", columns: ["val"]}
      ]},
      dec_proj_projected: %{user_id: "uid", db_name: "dec_proj_projected_base", projection: %{
        table: "dec_proj_enc", primary_key: "pk", foreign_key: "fk", user_id_alias: "uid"
      }},
    }

    data_source = Table.load(%{
  ...
  test "query on encoded table", context do
    assert_query context, "SELECT val FROM dec_proj_enc WHERE val = 10", %{rows: [%{occurrences: 10, row: [10]}]}
  end

  test "query on projected table", context do
    assert_query context, "SELECT count(*) FROM dec_proj_projected", %{rows: [%{occurrences: 1, row: [10]}]}
  end
