defmodule Cloak.Query.DBEmulatorTest do
  ...
  test "simple emulated subqueries" do
    ...
    assert_query "select count(v) from #{@vt}", """
    ...
    assert_query "select count(*) from #{@vt}", """
  ...
  describe "aggregation in emulated subqueries" do
    setup do
      :ok = insert_rows(_user_ids = 1..20, "#{@emulated_insert}",
    ...
    test "count(<column>)" do
      assert_query "select avg(v) from #{@vt}", """
    ...
    test "group by function" do
      assert_query "select v from #{@vt} order by v", """
    ...
    test "having inequality" do
      ...
      assert_query "select n + c from #{@vt} order by n + c", """
    ...
    test "nested having inequality" do
      ...
      assert_query "select n + c from #{@vt} order by n", """
    ...
    test "where inequality" do
      assert_query "select sum(c) from #{@vt}", """
    ...
    @tag pending: "mysterious indexing error"
    test "avg" do
      assert_query "select round(avg(v)) from #{@vt}", """
    ...
    @tag pending: "mysterious indexing error"
    test "stddev" do
      :ok = insert_rows(_user_ids = 21..21, "#{@emulated_insert}", ["value"], [Base.encode64("wasabi")])
      assert_query "select round(avg(v)) from #{@vt}", """
  ...
  describe "distinct in emulated subqueries" do
    ...
    test "select distinct", do:
      assert_query "select v from #{@vt}", "select distinct user_id, length(dec_b64(value)) as v from #{@emulated}",
        %{rows: [
  ...
  describe "emulated joins" do
    ...
    test "cross join", do:
      assert_query "select count(age) from #{@vt}, #{@joined} where #{@vt}.user_id = #{@joined}.user_id",
        "select * from #{@emulated}", %{rows: [%{occurrences: 1, row: [10]}]}

    test "inner join", do:
      assert_query """
        select count(v) from
          #{@vt} inner join (select user_id as uid from #{@joined}) as t on user_id = uid
      """, "select user_id, dec_b64(value) as v from #{@emulated}", %{rows: [%{occurrences: 1, row: [10]}]}

    test "left join between table and subquery" do
      assert_query """
        select count(v) from
          #{@vt} left join (select user_id as uid from #{@joined}) as t on user_id = uid
      """, "select user_id, dec_b64(value) as v from #{@emulated}", %{rows: [%{occurrences: 1, row: [20]}]}
    ...
    test "join with a row splitter function" do
      assert_query "select extract_words(value) from #{@joined} inner join #{@vt} on user_id = uid",
        "select user_id as uid, dec_b64(value) as value from #{@emulated}", %{rows: rows}
  ...
  describe "#{@emulated} subqueries with different case columns" do
    ...
    test "functions and conditions" do
      assert_query "select Value from #{@vt} where vALUE = 3",
    ...
    @tag pending: "crash in Validation.verify_all_uid_columns_are_compared_in_joins"
    test "join" do
      assert_query "select aGe, vaLue from #{@joined} inner join #{@vt} on uSer_Id = uId",
