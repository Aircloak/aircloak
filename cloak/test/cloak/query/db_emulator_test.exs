defmodule Cloak.Query.DBEmulatorTest do
  ...
  setup_all do
    :ok = Cloak.Test.DB.create_table("#{@prefix}emulated", "value TEXT, date TEXT, number INTEGER")
  ...
  test "simple emulated subqueries" do
    ...
    assert_query """
      select count(*) from (select user_id from #{@prefix}emulated order by dec_b64(value) limit 10) as t
    """, %{rows: [%{occurrences: 1, row: [10]}]}
    assert_query """
      select count(*) from (select user_id from #{@prefix}emulated order by dec_b64(value) limit 10 offset 10) as t
    """, %{rows: [%{occurrences: 1, row: [10]}]}
  end
  ...
  test "emulated subqueries with functions" do
    ...
    assert_query """
      select l from (select user_id, length(dec_b64(value)) as l from #{@prefix}emulated) as t order by l desc
    """, %{rows: [%{occurrences: 10, row: [3]}, %{occurrences: 10, row: [1]}]}
    assert_query """
      select v from (select user_id, dec_b64(value) as v from #{@prefix}emulated) as t where length(v) = 1
    """, %{rows: [%{occurrences: 10, row: ["x"]}]}
    assert_query """
      select v from (select user_id, dec_b64(value) AS v from #{@prefix}emulated where length(v) = 3) as t
  ...
  test "like on an emulated column" do
    ...
    assert_query "select count(*) from #{@prefix}emulated where dec_b64(value) like 'a_a%'", %{rows: [%{row: [20]}]}
  end
  ...
  describe "aggregation in emulated subqueries" do
    ...
    test "count(<column>)" do
      assert_query """
        select avg(v) from (select user_id, count(dec_b64(value)) as v from #{@prefix}emulated group by user_id) as t
    ...
    test "group by function" do
      assert_query """
        select v from
          (select user_id, dec_b64(value) as v from #{@prefix}emulated group by user_id, v) as t
        order by v
    ...
    test "where function" do
      assert_query """
          select avg(v) from (select user_id, count(dec_b64(value)) as v from
          #{@prefix}emulated where length(dec_b64(value)) = 3 group by user_id) as t
    ...
    test "having inequality" do
      ...
      assert_query """
        select n + c from
          (select user_id, number + 2 as n, count(dec_b64(value)) as c from #{@prefix}emulated
    ...
    test "where inequality" do
      assert_query """
        select sum(c) from
          (select user_id, count(dec_b64(value)) as c from #{@prefix}emulated
    ...
    test "having equality" do
      assert_query """
        select avg(v) from
          (select user_id, sum(length(dec_b64(value))) as v from #{@prefix}emulated
          group by user_id having max(length(dec_b64(value))) = 5) as t
    ...
    test "avg" do
      assert_query """
        select round(avg(v)) from
          (select user_id, avg(length(dec_b64(value))) as v from #{@prefix}emulated group by user_id) as t
    ...
    test "stddev" do
      ...
      assert_query """
        select round(avg(v)) from
          (select user_id, stddev(length(dec_b64(value))) as v from #{@prefix}emulated group by user_id) as t
    ...
    test "min/max/median with text" do
      assert_query """
        select * from (select user_id, min(dec_b64(value)), max(dec_b64(value)), median(dec_b64(value))
  ...
  describe "distinct in emulated subqueries" do
    ...
    test "count(distinct value)", do:
      assert_query """
        select avg(v) from (select user_id, count(distinct dec_b64(value)) as v
        from #{@prefix}emulated group by user_id) as t
      """, %{rows: [%{occurrences: 1, row: [5.0]}]}
    ...
    test "count(distinct length(value))", do:
      assert_query """
        select avg(v) from
          (select user_id, count(distinct length(dec_b64(value))) as v from #{@prefix}emulated group by user_id) as t
      """, %{rows: [%{occurrences: 1, row: [4.0]}]}

    test "select distinct", do:
      assert_query "select v from (select distinct user_id, length(dec_b64(value)) as v from #{@prefix}emulated) as t",
        %{rows: [
    ...
    test "avg(distinct)", do:
      assert_query """
        select avg(v) from
          (select user_id, avg(distinct length(dec_b64(value))) as v from #{@prefix}emulated group by user_id) as t
  ...
  describe "emulated joins" do
    ...
    test "left join between table and subquery" do
      assert_query """
        select count(dec_b64(value)) from
    ...
    test "join with a row splitter function" do
      assert_query """
        select extract_words(value) from #{@prefix}joined inner join
          (select user_id as uid, dec_b64(value) as value from #{@prefix}emulated) as t on user_id = uid
      """, %{rows: rows}
    ...
    test "where inequality" do
      ...
      assert_query """
        select count(*) - count(x.v) from
          (select user_id, dec_b64(value) as v from #{@prefix}emulated
    ...
    test "range in a join condition" do
      ...
      assert_query """
        select count(*) - count(v) from
          (select user_id, number, dec_b64(value) as v from #{@prefix}emulated) as t
          join #{@prefix}joined
    ...
    test "join between emulated subquery and subquery with aggregation", do:
      assert_query """
        ...
        (select user_id, dec_b64(value) as v from #{@prefix}emulated) as t1 inner join
    ...
    test "left join with filter in subquery" do
      assert_query """
        select value from (
          select #{@prefix}emulated.user_id, dec_b64(value) as value from
            #{@prefix}emulated left join (select user_id as uid from #{@prefix}joined) as t on user_id = uid
    ...
    test "left join with filter in top query" do
      assert_query """
        select dec_b64(value) from #{@prefix}emulated left join
          (select user_id as uid from #{@prefix}joined) as t on user_id = uid and dec_b64(value) = 'a b c'
        where t.uid is null
      """, %{rows: [%{occurrences: 10, row: ["a b c"]}]}
    end
  end
  ...
  test "#{@prefix}emulated subqueries with different case columns" do
    ...
    assert_query "select Value from (select user_id, dec_b64(vaLue) as value from #{@prefix}emulated) as t",
      %{rows: [%{occurrences: 10, row: ["abc"]}]}

    assert_query """
      select Value from (select user_id, length(dec_b64(vaLue)) as vAlue from #{@prefix}emulated) as t where vALUE = 3
    """, %{rows: [%{occurrences: 10, row: [3]}]}

    assert_query """
      select aGe, vaLue from #{@prefix}joined inner join
      (select user_Id as Uid, dec_b64(value) as Value from #{@prefix}emulated group by User_id, VaLue) as t
      on uSer_Id = uId
  ...
  test "[BUG] join with alias and decoded column" do
    ...
    assert_query """
      select count(*) - count(v) from #{@prefix}joined as t1 inner join
        (select user_id, dec_b64(value) as v, number as n from #{@prefix}emulated) as t2
        on t1.user_id = t2.user_id and t1.age = t2.n
    """, %{rows: [%{row: [10]}]}
  end

  test "[BUG]: multiple JOINs with complex filter on encoded columns" do
    :ok = insert_rows(_user_ids = 1..10, "#{@prefix}emulated", ["value"], [Base.encode64("10")])
    :ok = insert_rows(_user_ids = 1..10, "#{@prefix}joined", ["age"], [10])

    assert_query """
      select count(*) from (
        select t1.user_id from #{@prefix}joined as t1 join #{@prefix}emulated as t2
        on t1.user_id = t2.user_id where dec_b64(value) in ('10', '20')
