defmodule Cloak.Query.ShrinkAndDrop.Test do
  ...
  alias Cloak.Aql.{Column, Range}
  alias Cloak.Query.ShrinkAndDrop

  test "when the stream is empty the result is empty" do
    data = []
    query = %{ranges: [Range.new(%Column{type: :integer, db_row_position: 1}, {0, 20}, :having)]}
  ...
  test "a basic scenario" do
    ...
    query = %{ranges: [Range.new(%Column{type: :integer, db_row_position: 1}, {0, 50}, :having)]}
