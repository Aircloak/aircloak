defmodule Cloak.Query.ShrinkAndDrop.Test do
  ...
  describe "HalfBuffer" do
    test "adding to an empty buffer" do
      assert HalfBuffer.add(HalfBuffer.new(3), 10, "user1", :some_data, :not_used) ==
        {%HalfBuffer{size: 3, min: 10, max: 10, users: %{"user1" => %{value: 10, rows: [:some_data]}}}, []}
    end

    test "adding when the user already in the buffer" do
      buffer = %HalfBuffer{size: 3, min: 10, max: 20, users: %{"user1" => %{value: 20, rows: [:some_data]}}}
      assert HalfBuffer.add(buffer, 25, "user1", :more_data, &Kernel.>/2) ==
        {%HalfBuffer{size: 3, min: 10, max: 25, users: %{"user1" => %{value: 25, rows: [:more_data, :some_data]}}}, []}
    end

    test "adding when the user is not in the buffer" do
      buffer = %HalfBuffer{size: 3, min: 10, max: 20, users: %{"user1" => :some_data}}
      assert HalfBuffer.add(buffer, 5, "user2", :more_data, :not_used) ==
    ...
    test "adding to a full buffer" do
      buffer = %HalfBuffer{size: 3, min: 1, max: 3, users: %{
        "user1" => %{value: 1, rows: [:some_data]},
        "user2" => %{value: 2, rows: [:other_data]},
        "user3" => %{value: 3, rows: [:more_data]},
      }}

      assert HalfBuffer.add(buffer, 0, "user4", :even_more_data, &Kernel.</2) ==
        {
