defmodule Cloak.Query.VirtualTableTest do
  ...
  setup_all do
    ...
    Cloak.Test.DB.create_table("projected_transactions_real",
      "id INTEGER, account_id INTEGER, amount INTEGER", add_user_id: false)
    Cloak.Test.DB.create_table("projected_notes_real", "transaction_id INTEGER, note TEXT", add_user_id: false)
  ...
  defp insert_transaction(id, amount, note) do
    Cloak.Test.DB.insert_data("projected_transactions_real", ["id", "account_id", "amount"], [[id, id, amount]])
    Cloak.Test.DB.insert_data("projected_notes_real", ["transaction_id", "note"], [[id, "(1) " <> note]])
    Cloak.Test.DB.insert_data("projected_notes_real", ["transaction_id", "note"], [[id, "(2) " <> note]])
  end

  defp virtual_table_error(query), do:
    capture_log(fn -> create_virtual_table!("vtt_fake", query) end)
  ...
  test "virtual table with subquery" do
    create_virtual_table!("vtt_fake", "select * from (select user_id, ival from cloak_test.vtt_real) as t")
  ...
  test "[Issue #1538] joins with virtual tables are order-independent" do
    ...
    :ok = Cloak.Test.DB.create_table("vtt_join_real1", "row_id INTEGER, one_id INTEGER", add_user_id: false)
    :ok = Cloak.Test.DB.create_table("vtt_join_real2", "one_id INTEGER, two_id INTEGER", add_user_id: false)
  ...
  describe "table projection tests" do
    setup do
      ...
      """)
      create_virtual_table!("projected_notes", """
