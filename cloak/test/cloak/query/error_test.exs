defmodule Cloak.Query.ErrorTest do
  ...
  test "query reports an error on invalid where clause identifier" do
    assert_query "select height from test_errors where nonexistant > 10", %{error: %{human_description: error}}
  ...
  test "query reports an error on invalid order by field" do
    assert_query "select height from test_errors order by name", %{error: %{human_description: error}}
    assert ~s/Non-selected column specified in `ORDER BY` clause./ == error
  end

  test "query reports an error on unknown function" do
    assert_query "select invalid_function(height) from test_errors", %{error: %{human_description: error}}
    assert ~s/Unknown function `invalid_function`./ == error
  end

  test "reports an error on wrong cast" do
    assert_query "select * from test_errors where datetime > 0", %{error: %{human_description: error}}
  ...
  test "reports an error on collision between alias and column in group by" do
    assert_query "select abs(height) as height from test_errors group by height",
      %{error: %{human_description: error}}
    assert ~s/Usage of `height` is ambiguous./ == error
  end

  test "query reports an error on invalid statement" do
    assert_query "invalid statement", %{error: %{human_description: "Expected `select or show` at line 1, column 1."}}
  end

  test "query reports an error on invalid column" do
    assert_query "select invalid_column from test_errors", %{error: %{human_description: error}}
    assert ~s/Column `invalid_column` doesn't exist in table `test_errors`./ == error
  end

  test "query reports an error on invalid table" do
    assert_query "select column from invalid_table", %{error: %{human_description: error}}
    assert ~s/Table `invalid_table` doesn't exist./ == error
  end

  test "query reports an error when mixing aggregated and normal columns" do
    assert_query "select count(*), height from test_errors", %{error: %{human_description: error}}
  ...
  test "query reports an error when grouping by nonexistent columns" do
    assert_query "select count(*) from test_errors group by nothing", %{error: %{human_description: error}}
  ...
  test "query reports an error when not grouping by some selected columns" do
    assert_query "select name, height from test_errors group by height", %{error: %{human_description: error}}
  ...
  test "query reports an error on runner crash" do
    ExUnit.CaptureLog.capture_log(fn ->
      assert_query :invalid_query_type, %{error: %{human_description: "Unknown cloak error."}}
    end)
  end

  test "substring with neither for nor from" do
    assert_query "select substring(name) from test_errors", %{error: %{human_description: error}}
  ...
  test "query reports error on invalid limit / offset parameters" do
    assert_query "select name from test_errors limit -1", %{error: %{human_description: error}}
    assert ~s/`LIMIT` clause expects a positive value./ == error
    assert_query "select name from test_errors offset -1", %{error: %{human_description: error}}
  ...
  test "query reports error on invalid having clause" do
    ...
    assert_query "select name from test_errors having count(*) >= 10", %{error: %{human_description: error}}
  ...
  test "query reports error on invalid where clause" do
    assert_query "select name from test_errors where max(height) >= 100", %{error: %{human_description: error}}
