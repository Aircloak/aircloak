    ...
    defmodule Module.concat([Compliance.AggregateFunctions, String.to_atom(aggregate), Test]) do
      ...
      Enum.each(numerical_columns(), fn {column, table, uid} ->
        ...
        test "aggregate #{aggregate} on input #{column} in query on #{table}", context do
          ...
          # `*_noise` functions on userless tables are replaced with `GROUP BY constant`,
          # something which SQL Server and Drill don't like.
          |> disable_for(Cloak.DataSource.SQLServer, function =~ ~r/_noise/ and unquote(table) == "users_public")
          |> disable_for(Cloak.DataSource.Drill, function =~ ~r/_noise/ and unquote(table) == "users_public")
          |> assert_consistent_and_not_failing("""
