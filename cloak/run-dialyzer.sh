...
# the travis output.
if [ ! -f $HOME/cloak-plt/dialyzer.plt ]; then
  echo "rebuidling base plt"
  dialyzer --build_plt --apps erts kernel stdlib crypto sasl --output_plt $HOME/cloak-plt/dialyzer.plt &> /dev/null
fi

echo "rebuilding deps plt"
REBAR_PLT_DIR="$HOME/cloak-plt" make rebuild-plt &> /dev/null

echo "dialyzing cloak"
env REBAR_PLT_DIR="$HOME/cloak-plt" make dialyzer
