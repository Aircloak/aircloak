...
operation can safely be performed with a builtin operator. The datasource adapters are free to implement these
`unsafe_*` operations with the native operators.

The process of bound analysis takes form of a series of rules for extending the bounds of operands onto an operation.
For example, if `a` is in the bounds `{Amin, Amax}` and `b` is in the bounds `{Bmin, Bmax}`, then `a + b` is known to be
in the bounds `{Amin + Bmin, Amax + Bmax}`.
...
5. Take the top/bottom value after that and find the closest money-aligned value above/below it
