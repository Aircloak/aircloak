...
-spec anonymize([#bucket_report{}]) -> [#bucket_report{}].
anonymize(AggregatedReports) ->
  anonymize(AggregatedReports, undefined).
...
%%
%%        - `#bucket_report.count':  this is the raw count of the users in the bucket without any noise added.
%%
%%        - `#bucket_report.noisy_count':  this is the noisy count we will report to the air (it will get
%%          constructed in many steps by `anonymize/1', thus the record value will contain temporary values
%%          while running `anonymize/1' and has to be initialized to #bucket_report.count).
%%
...
%%          noise added to create `#bucket_report.noisy_count'.
%%
%%        - `#anonymizer_params.max_sigma': a floating point value representing the maximum standard deviation of
%%          noise added to create `#bucket_report.noisy_count'. This value is a soft bound. Exceeding it does not
%%          damage to privacy, but might hurt utility.
%%
%%      The following properties are fulfilled by the resulting buckets:
%%
%%        - `#bucket_report.count > #anonymizer_params.absolute_lower_bound'
%%
%%        - #bucket_report.count + noise with sigma `#anonymizer_params.sigma_soft_lower_bound' > `#anonymizer_params.soft_lower_bound'.
%%          Remark: The noise used in the soft lower bound comparison is computed independently of the noise used to generate the
%%          noisy counts that are reported. Thus the final reported `#bucket_report.noisy_count' could be less than
%%          `#anonymizer_params.soft_lower_bound'. If the noisy count is rounded down to 0 it will be filtered out though.
%%
%%        - Noise is proportional to the `#bucket_report.count', but we guarantee that at least noise with a
%%          standard deviation of `#anonymizer_params.min_sigma' is added.
%%
%%        - `#bucket_report.noisy_count > 0'.
%%
%%        - Each bucket returned by `anonymize/1' existed in the bucket list given to `anonymize/1'.
%%
%%        - `#bucket_report.noisy_count mod K ≡ 0' with `K' equals 5 or 10 depending on the noise added.
-spec anonymize([#bucket_report{}], undefined | lcf_users:lcf_users()) -> [#bucket_report{}].
anonymize(AggregatedReports, LcfUsers) ->
  ReportsWithAnonState = [append_anonymization_state(Report) || Report <- AggregatedReports],
...
lcf_tail_reports(LcfFilteredLabels, Params, LcfUsers) ->
  case lcf_users:lcf_tail_report(LcfUsers, LcfFilteredLabels) of
    undefined -> [];
    LcfTailReport ->
      {Passed, _Filtered} = filter_lcf([append_anonymization_state(LcfTailReport)], Params),
      Passed
  end.

%% @doc Wraps the bucket report with an anonymization state
%%      that is used internally while anonymizing.
append_anonymization_state(#bucket_report{users_hash=UsersHash}=Report) ->
  <<A:32, B:32, C:64>> = UsersHash,
  AnonymizationState = #anonymization_state{userids_hash={A,B,C}},
  {Report, AnonymizationState}.

strip_anonymization_state({Report, _AnonymizationState}) -> Report.
...
%%      users.
absolute_low_count_filter({#bucket_report{count=Count}, _State},
    #anonymizer_params{absolute_lower_bound=LowerBound}) when Count =< LowerBound ->
  failed;
absolute_low_count_filter(Report, _) -> Report.
...
soft_low_count_filter({#bucket_report{count=Count}, #anonymization_state{userids_hash=UserIdHashSeed}}=FullReport,
...
apply_constant_noise({#bucket_report{noisy_count=Count}=Bucket,
  ...
  UpdatedBucket = Bucket#bucket_report{noisy_count=NoisedCount},
...
apply_proportional_random_noise({#bucket_report{noisy_count=Count}=Bucket, AnonState}, AnonParams) ->
  Sigma = noise_sigma_for_count(Count, AnonParams),
  NoisyCount = cloak_distributions:gauss(Sigma, Count),
  {Bucket#bucket_report{noisy_count=NoisyCount}, remember_noise(Sigma, AnonState)}.
...
calculate_total_noise({Bucket, #anonymization_state{noise_sds=NoiseValues}=AnonState}, _AnonymizationParameters) ->
  ...
  {Bucket#bucket_report{noise_sd=TotalNoise}, AnonState}.
...
cap_noise_to_4sd({#bucket_report{noise_sd=TotalNoiseSD, count=RawCount, noisy_count=NoisyCount}=Bucket,
  ...
  AdjustedBucket = case abs(RawCount - NoisyCount) > 4 * TotalNoiseSD of
    true ->
      ...
      Bucket#bucket_report{noisy_count = RawCount + (Sign * TotalNoiseSD * 4)};
    false ->
      Bucket
  end,
...
round_count({#bucket_report{noisy_count=Count, noise_sd=Sigma}=Bucket, AnonState}, _AnonymizationParameters) ->
  {Bucket#bucket_report{noisy_count=round_noisy(Count, Sigma)}, AnonState}.

%% @doc Remove buckets with a non-positive noisy count.
remove_non_positive_buckets({#bucket_report{noisy_count=Count}=Bucket, AnonState}, _AnonymizationParameters) ->
  if
    Count =< 0 ->
      failed;
    Count > 0 ->
...
-define(report(Count), {bucket_report(Count), anon_params()}).
-define(
  report(Count, NoisyCount),
  {
    (bucket_report(Count))#bucket_report{noisy_count=NoisyCount},
    anon_params()
  }
).
-define(
  report(Count, NoisyCount, NoiseSD),
  {
    ((bucket_report(Count))#bucket_report{noisy_count=NoisyCount})#bucket_report{noise_sd=NoiseSD},
    anon_params()
  }
).
-define(
  report(Count, NoisyCount, NoiseSD, UsedSDs),
  {
    ((bucket_report(Count))#bucket_report{noisy_count=NoisyCount})#bucket_report{noise_sd=NoiseSD},
    anon_params(UsedSDs)
  }
...
apply_constant_noise_test() ->
  F = fun(_) -> ?assertEqual(?report(4, 4, 0, [1]), ?run(apply_constant_noise, ?report(4))) end,
  lists:foreach(F, lists:seq(1,100)).

absolute_low_count_filter_test_() ->
  [
    ?_assertEqual(failed, ?run(absolute_low_count_filter, ?report(0))),
    ?_assertEqual(failed, ?run(absolute_low_count_filter, ?report(1))),
    ?_assertEqual(?report(2), ?run(absolute_low_count_filter,  ?report(2)))
  ].
...
soft_low_count_filter_test_() ->
  ...
  [
    ?_assertEqual(failed, ?run(soft_low_count_filter, ?report(0))),
    ?_assertEqual(failed, ?run(soft_low_count_filter, ?report(4))),
    ?_assertEqual(?report(5), ?run(soft_low_count_filter,  ?report(5)))
  ]}.
...
noise_test_() ->
  {setup,
    ...
    [
      {"Constant noise", [
        ?_assertEqual(?report(4, 5, 0, [1]), ?run(apply_constant_noise, ?report(4))),
        ?_assertEqual(?report(5, 6, 0, [1]), ?run(apply_constant_noise, ?report(5)))
      ]},
      {"Variable noise", [
        {"Sigma = sqrt(3) for 120 users and target error 5%",
           ?_assertEqual(?report(120, 122, 0, [math:sqrt(3)]), ?run(apply_proportional_random_noise, ?report(120)))
        },
        {"Sigma = sqrt(15) for 240 users and target error 5%",
           ?_assertEqual(?report(240, 244, 0, [math:sqrt(15)]), ?run(apply_proportional_random_noise, ?report(240)))
        },
        {"Sigma ~= 20 (sqrt(400 - 1) because of static noise = 1) for 999999 users, because it is capped",
           ?_assertEqual(?report(999999, 1000019, 0, [
              math:sqrt(?MAX_SIGMA * ?MAX_SIGMA - ?SIGMA_SOFT_LOWER_BOUND * ?SIGMA_SOFT_LOWER_BOUND)]),
              ?run(apply_proportional_random_noise, ?report(999999)))
        },
        {"Fall back to min sigma = 1 for 30 users and target error 5%",
           ?_assertEqual(?report(30, 31, 0, [?MIN_SIGMA]), ?run(apply_proportional_random_noise, ?report(30)))
        }
      ]}
    ]
  }.
...
calculate_noise_total_test_() ->
  [
    ?_assertEqual(?report(0, 0, 4.0, [2,2,2,2]), ?run(calculate_total_noise, ?report(0, 0, 0, [2,2,2,2]))),
    ?_assertEqual(?report(0, 0, 6.0, [4,4,2]), ?run(calculate_total_noise, ?report(0, 0, 0, [4,4,2])))
  ].

cap_noise_to_4sd_test_() ->
  [
    ?_assertEqual(?report(100, 120, 5), ?run(cap_noise_to_4sd, ?report(100, 130, 5))),
    ?_assertEqual(?report(100, 80, 5), ?run(cap_noise_to_4sd, ?report(100, 5, 5))),
    ?_assertEqual(?report(100, 110, 5), ?run(cap_noise_to_4sd, ?report(100, 110, 5))),
    ?_assertEqual(?report(100, 90, 5), ?run(cap_noise_to_4sd, ?report(100, 90, 5)))
  ].

round_count_test_() ->
  [
    ?_assertEqual(?report(0, 15, 4), ?run(round_count, ?report(0, 15, 4))),
    ?_assertEqual(?report(0, 10, 4), ?run(round_count, ?report(0, 12, 4))),
    ?_assertEqual(?report(0, 20, 5), ?run(round_count, ?report(0, 15, 5))),
    ?_assertEqual(?report(0, 10, 5), ?run(round_count, ?report(0, 14, 5)))
  ].

remove_non_positive_buckets_test_() ->
  [
    ?_assertEqual(failed, ?run(remove_non_positive_buckets, ?report(0, 0))),
    ?_assertEqual(?report(0, 1), ?run(remove_non_positive_buckets, ?report(0, 1))),
    ?_assertEqual(failed, ?run(remove_non_positive_buckets, ?report(0, -1))),
    ?_assertEqual(?report(0, 2), ?run(remove_non_positive_buckets, ?report(0, 2)))
  ].
  ...
  {setup,
    ...
    [
      {"Too small counts get filtered out", [
        ?_assertEqual([], anonymize([?strip(?report(0)), ?strip(?report(1)), ?strip(?report(2))]))
      ]},
      ...
      {"Values that fall below the soft threshold after noise should be removed", [
        ...
        %% The soft threshold that should be exceeded in order to get a reported value is 7 (5 + 2)
        ?_assertEqual([], anonymize([?strip(?report(2)), ?strip(?report(3))]))
      ]},

      {"Noise should be applied and answers rounded", [
        ?_assertEqual([?strip(?report(100, 105, 2.5))], anonymize([?strip(?report(100))])),
        ?_assertEqual([?strip(?report(120, 125, 2.5))], anonymize([?strip(?report(120))])),
        ?_assertEqual([?strip(?report(400, 405, 2.5))], anonymize([?strip(?report(400))])),
        ?_assertEqual([?strip(?report(1800, 1810, 6.006666666666668))], anonymize([?strip(?report(1800))]))
      ]}
    ]
  }.
...
target_error_test() ->
  ...
  Buckets = [bucket_report(750), bucket_report(997), bucket_report(1567), bucket_report(2345), bucket_report(8345)],
...
lcf_test_() ->
  {
    ...
    [
      {"lcf tail is reported", ?_assertEqual(8, (lcf_tail([{p1, 1, 4}, {p2, 5, 8}]))#bucket_report.count)},
      {"lcf tail count has noise",
        % Although we have 8 users, the noisy count should be 10, due to discretization
        ?_assertEqual(10, (lcf_tail([{p3, 11, 14}, {p4, 15, 18}]))#bucket_report.noisy_count)},
      ...
      {"users are deduplicated", ?_assertEqual(7, (lcf_tail([{p6, 31, 34}, {p7, 34, 37}]))#bucket_report.count)},
      % A large bucket including all users shouldn't affect the lcf tail
      {"only lcf-ed properties are included in lcf tail",
        ?_assertEqual(8, (lcf_tail([{p8, 41, 44}, {p9, 45, 48}, {p10, 41, 100}]))#bucket_report.count)}
    ]
  }.
...
lcf_tail(Properties) ->
  ...
  try
    ...
    case [Report ||
      #bucket_report{
        label=#bucket_label{label=?AIRCLOAK_LABEL, value=?LCF_TAIL_VALUE}
      } = Report <- anonymizer:anonymize(aggregator:reports(Aggregator), LcfUsers)
    ] of
      [] -> undefined;
      [Count] -> Count
    end
