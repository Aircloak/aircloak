defmodule AircloakCI do
  ...
  def data_folder(), do: Path.join(home_folder(), "data")

  @doc "Returns the github personal access token used by this service."
  @spec github_token() :: {:ok, String.t()} | {:error, String.t()}
  def github_token() do
    with {:ok, config} <- user_config(), do: config_value(config, "github_access_token")
  end
  ...
  defp user_config() do
    with :error <- Application.fetch_env(:aircloak_ci, :user_config),
         {:ok, config} <- read_user_config() do
  ...
  defp config_value(config, key) do
    with :error <- Map.fetch(config, key), do: {:error, "missing config `#{key}`"}
  end

  defp config_file(), do: Path.join(home_folder(), "config.json")
