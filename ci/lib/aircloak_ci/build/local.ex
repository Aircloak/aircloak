defmodule AircloakCI.Build.Local do
  ...
  def build_source(path, repo_data),
    do: %{
      ...
      base_branch: nil,
      project: LocalProject.for_local(path)
    }

  @impl Build.Server
  def init(pid, state), do: {:ok, %{state | data: pid}}

  # -------------------------------------------------------------------
  # Internal functions
  # -------------------------------------------------------------------

  defp name(), do: {:via, Registry, {AircloakCI.Build.Registry, :local}}

  defp source(path, repo_data),
    do: %{
      path: path,
  ...
  defp empty_repo(), do: %{owner: "aircloak", name: "aircloak", branches: [], pull_requests: []}
  ...
  @doc false
  def start_link(path, caller),
    do: Build.Server.start_link(__MODULE__, :local, path, empty_repo(), caller, name: name())
end
