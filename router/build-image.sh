#!/usr/bin/env bash

set -eo pipefail

cd $(dirname $0)
. ../common/docker_helper.sh

function log {
  msg=$1
  echo "[aircloak] $msg"
}

cp -rp ../config/config.sh docker
cp -rp ../config/tcp_ports.json docker

# -------------------------------------------------------------------
# Docker release image build
# -------------------------------------------------------------------

log "Building the router image"
setup_env_init
docker build -t aircloak/air_router:latest .

if named_container_running air_docker_registry; then
  log "Pushing to local registry"
  docker tag -f aircloak/air_router:latest localhost:5000/aircloak/air_router:latest
  docker push localhost:5000/aircloak/air_router:latest
else
  echo "Warning: local registry is not running, image not pushed."
fi
