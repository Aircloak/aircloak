defmodule BOM.Validate.Test do
  ...
  test "package with no name", do:
    assert BOM.Validate.run(%{@valid_package | name: nil}).error == "Name empty"

  test "package with empty name", do:
    assert BOM.Validate.run(%{@valid_package | name: ""}).error == "Name empty"

  test "package with no license", do:
    assert BOM.Validate.run(%{@valid_package | license: nil}).error == "No license"
