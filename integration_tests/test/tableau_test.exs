defmodule IntegrationTest.TableauTest do
  ...
  test "tableau table list query", context do
    query = 'BEGIN;declare "SQL_CUR04645D10" cursor for select relname, nspname, relkind from pg_catalog.pg_class c, pg_catalog.pg_namespace n where relkind in (\'r\', \'v\') and nspname not in (\'pg_catalog\', \'information_schema\', \'pg_toast\', \'pg_temp_1\') and n.oid = relnamespace order by nspname, relname;fetch 2048 in "SQL_CUR04645D10";'
  ...
  test "tableau query for related fields", context do
    query = 'BEGIN;declare "SQL_CUR06145EE8" cursor for select c.relname, i.indkey, i.indisunique, i.indisclustered, a.amname, c.relhasrules, n.nspname, c.oid, d.relhasoids, 0 from pg_catalog.pg_index i, pg_catalog.pg_class c, pg_catalog.pg_class d, pg_catalog.pg_am a, pg_catalog.pg_namespace n where d.relname = \'transactions\' and n.nspname = \'public\' and n.oid = d.relnamespace and d.oid = i.indrelid and i.indexrelid = c.oid and c.relam = a.oid order by i.indisprimary desc, i.indisunique, n.nspname, c.relname;fetch 2048 in "SQL_CUR06145EE8"'
    assert :odbc.sql_query(context.conn, query) == [
      {:updated, 0},
      {:updated, 0},
      {:selected,
        ['relname', 'indkey', 'indisunique', 'indisclustered', 'amname', 'relhasrules', 'nspname', 'oid', 'relhasoids', '?column?'],
        []
      }
    ]
  end
  ...
  test "select current schema", context, do:
    assert :odbc.sql_query(context.conn, 'select current_schema()') == {:selected, ['current_schema'], [{''}]}
  ...
  test "show lc_collate", context, do:
    assert {:ok, %Postgrex.Result{columns: ["lc_collate"], command: :select, rows: [["C"]]}} =
