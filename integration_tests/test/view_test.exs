defmodule IntegrationTest.ViewTest do
  ...
  test "name error", context do
    assert {:error, %Ecto.Changeset{errors: [name: {error, _}]}} =
             create_view(context.user, "users", "sql")

    assert error == "has already been taken"
  end

  test "sql syntax error", context do
    assert {:error, %Ecto.Changeset{errors: [sql: {error, _}]}} =
             create_view(context.user, "foo", "select")
  ...
  test "successful saving of the new view", context do
    assert {:ok, view} =
             create_view(context.user, unique_view_name(), "select user_id, name from users")
  ...
  test "cannot insert two views with the same name", context do
    ...
    assert {:error, changeset} =
             create_view(context.user, view.name, "select user_id, name from users")
  ...
  defp create_view(user, name, sql),
    do: Air.Service.View.create(user, Manager.data_source(), name, sql)

  defp update_view(view_id, user, name, sql),
    do: Air.Service.View.update(view_id, user, name, sql)
