...
```
The `name` parameter is a string which will be used to identiy the data source throughout the Air interface and APIs. The `driver` parameter can be one of the following: `mongodb`, `postgresql`, `mysql`, `sqlserver`, `hana`, `odbc`. Next, you need to specify the database connection parameters.
