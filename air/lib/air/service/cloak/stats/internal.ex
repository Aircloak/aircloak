defmodule Air.Service.Cloak.Stats.Internal do
  ...
  def record_memory(%{stats: stats} = state, cloak_id, reading) do
    updated_stats = Map.update(stats, cloak_id, initial_memory_stats(reading), &update_base_memory_stats(&1, reading))

    state
    |> put_in([:stats], updated_stats)
  ...
  defp update_base_memory_stats(stats, reading),
    do: update_in(stats, [:memory], &set_base_for_memory_reading(&1, reading))

  defp initial_memory_stats(reading) do
    readings = List.duplicate(0, @timesteps_to_keep)
    %{memory: set_base_for_memory_reading(%{readings: readings}, reading)}
  end

  defp add_to_list_of_readings(readings, reading), do: Enum.take([reading | readings], @timesteps_to_keep)

  defp set_base_for_memory_reading(mem_stats, reading) do
    {total, in_use, in_use_percent} = base_stats(reading)
    Map.merge(mem_stats, %{total: total, currently_in_use: in_use, in_use_percent: in_use_percent})
  end
