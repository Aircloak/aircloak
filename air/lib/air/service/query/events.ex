defmodule Air.Service.Query.Events do
  ...
  def unsubscribe(query_id), do: Registry.unregister(__MODULE__.Registry, {:subscriber, query_id})
  ...
  def trigger_result(payload) do
    for {pid, _} <- Registry.lookup(__MODULE__.Registry, {:subscriber, payload.query_id}),
        do: send(pid, {:query_result, payload})
  ...
  def trigger_state_change(payload) do
    for {pid, _} <- Registry.lookup(__MODULE__.Registry, {:subscriber, payload.query_id}),
        do: send(pid, {:query_state_change, payload})
