defmodule Air.Service.Central.RpcQueue do
  ...
  def push(pid \\ __MODULE__, event, payload), do: GenServer.call(pid, {:push, event, payload})
  ...
  defp handle_push(state, event, payload),
    do:
      state
      |> update_in([:queue], &Aircloak.Queue.push(&1, new_rpc(event, payload)))
      |> update_in(
        [:queue],
        &Aircloak.Queue.drop_while(&1, fn queue -> queue.size > state.options.max_size end)
      )
      |> maybe_start_rpc_task()

  defp new_rpc(event, payload),
    do:
  ...
  defp send_to_central(rpc), do: {:ok, _} = Air.CentralClient.Socket.rpc(rpc)

  defp send_finished(state, reason),
    do:
      state
  ...
  defp handle_finished_reason(state, :normal),
    do: %{state | queue: Aircloak.Queue.drop_if(state.queue, &(&1 == state.current_send.rpc))}
