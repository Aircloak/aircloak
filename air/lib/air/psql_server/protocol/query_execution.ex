defmodule Air.PsqlServer.Protocol.QueryExecution do
  ...
  def handle_event(
        %{describing_statement: {describe_type, name}} = protocol,
    ...
    protocol
    |> put_in(
      [describe_storage, name, :parsed_param_types],
      Keyword.fetch!(description, :param_types)
    )
  ...
  defp send_parameter_descriptions(protocol, %{params: nil}, param_types),
    # parameters are not bound -> send parameter descriptions
    do: Protocol.send_to_client(protocol, {:parameter_description, param_types})

  defp send_parameter_descriptions(protocol, _, _),
  ...
  defp send_notices(protocol, info_messages),
    do: Enum.reduce(info_messages, protocol, &Protocol.send_to_client(&2, {:notice, &1}))

  defp send_command_completion(protocol, {:error, _}), do: protocol
  ...
  defp command_atom_to_string(tag_atom),
    do:
      tag_atom
      |> Atom.to_string()
      |> String.upcase()
  ...
  defp send_rows(protocol, rows, columns, formats) do
    ...
    Enum.reduce(
      rows,
      protocol,
      &Protocol.send_to_client(&2, {:data_row, &1, column_types(columns), formats})
    )
  end
  ...
  defp params_with_types(%{params: nil}), do: nil

  defp params_with_types(prepared_statement) do
    param_types =
      cond do
        match?([_ | _], prepared_statement.param_types) ->
          prepared_statement.param_types

        match?([_ | _], prepared_statement.parsed_param_types) ->
          prepared_statement.parsed_param_types

        true ->
          Stream.cycle([:unknown])
      end
