defmodule Air.PsqlServer.Protocol.ConnectionSetup do
  ...
  @impl Protocol
  def handle_client_message(%{state: :initial} = protocol, :raw, message),
    do:
  ...
  def handle_client_message(%{state: :ssl_negotiated} = protocol, :raw, message),
    do: handle_intent_message(protocol, message)

  def handle_client_message(%{state: :cancelling_query} = protocol, :raw, message),
    do:
      protocol
  ...
  @impl Protocol
  def handle_event(%{state: :negotiating_ssl} = protocol, :ssl_negotiated),
    do: Protocol.await_client_message(protocol, state: :ssl_negotiated, bytes: 8, decode?: false)
  ...
  defp handle_initial_message(protocol, message, %{msg_type: :startup, ssl_required: false}),
    # TCP connection and SSL is not required -> proceed
    do: handle_intent_message(protocol, message)

  defp handle_initial_message(protocol, _message, %{msg_type: :ssl, ssl_valid: :ok}),
  ...
  defp handle_initial_message(protocol, _message, %{
         msg_type: :ssl,
         ssl_valid: {:error, _},
         ssl_required: false
       }),
  ...
  defp handle_initial_message(protocol, _message, %{msg_type: :startup, ssl_required: true}),
