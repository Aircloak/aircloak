defmodule Air.PsqlServer.Protocol.Authentication do
  ...
  @impl Protocol
  def handle_client_message(%{state: :login_params} = protocol, :raw, raw_login_params),
    do:
      protocol
      |> Protocol.add_action({:login_params, Messages.decode_login_params(raw_login_params)})
      |> Protocol.next_state(:authenticating)

  def handle_client_message(protocol, :password, password),
    do:
      protocol
  ...
  @impl Protocol
  def handle_event(%{state: :authenticating} = protocol, {:authentication_method, method}),
    do:
      protocol
  ...
  def handle_event(%{state: :authenticating} = protocol, {:authenticated, true}) do
    key_data = Air.PsqlServer.ConnectionRegistry.register()

    protocol
    |> Protocol.send_to_client(:authentication_ok)
    |> Protocol.send_to_client({:parameter_status, "application_name", "aircloak"})
    |> Protocol.send_to_client({:parameter_status, "server_version", "12.1"})
  ...
  def handle_event(%{state: :authenticating} = protocol, {:authenticated, false}),
    do:
      protocol
