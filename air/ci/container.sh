...
function prepare_for_test {
  ...
  docker run \
    ...
    postgres:11.2 -c "listen_addresses=*" > /dev/null
...
mount_to_component \
  .flowconfig .gitignore assets flow-typed config datagen include lib perftest priv rel test mix.exs mix.lock Makefile \
