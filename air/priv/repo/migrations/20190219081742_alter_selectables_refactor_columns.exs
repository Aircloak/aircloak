defmodule Air.Repo.Migrations.AlterSelectablesRefactorColumns do
  use Ecto.Migration
  ...
  def up do
    ...
    alter(table(:user_selectables), do: add(:columns, :map, default: "[]"))
    execute("update user_selectables set columns = (result_info->>'columns')::jsonb")
    alter(table(:user_selectables), do: remove(:result_info))
  ...
  def down do
    ...
    alter(table(:user_selectables), do: add(:result_info, :map, default: "{}"))
    execute("update user_selectables set result_info = json_build_object('columns', columns)::jsonb")
    alter(table(:user_selectables), do: remove(:columns))
