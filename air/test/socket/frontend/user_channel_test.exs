defmodule Air.Socket.Frontend.UserChannelTest do
  ...
  describe "session channel" do
    setup do
      ...
      {:ok, socket: socket("user", %{user: user}), user: user, session: session}
    end
    ...
    test "results of queries are pushed to the given session", %{user: user, socket: socket, session: session_id} do
      {:ok, _, _} = subscribe_and_join(socket, UserChannel, "session:" <> session_id)
    ...
    test "results of queries are not pushed to other sessions", %{user: user, socket: socket, session: session_id} do
      {:ok, _, _} = subscribe_and_join(socket, UserChannel, "session:" <> session_id)
