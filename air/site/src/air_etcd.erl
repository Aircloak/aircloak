...
url() ->
  case application:get_env(air_common, etcd_url) of
    {ok, EtcdUrl} -> EtcdUrl;
    undefined ->
      {ok, EtcdPort} = application:get_env(air, etcd_port),
      EtcdUrl = lists:flatten(io_lib:format("http://127.0.0.1:~p", [EtcdPort])),
...
decode_response({ok, _Status, _Headers, ClientRef}) ->
  ...
  {ok, Response} = 'Elixir.Poison':decode(ResponseBody),
  {ok, Response}.
...
handle_etcd_response(Response) ->
  case maps:find(<<"errorCode">>, Response) of
    error -> ok;
    {ok, ?ETCD_COMPARE_FAILED} -> error;
    {ok, ?ETCD_KEY_ALREADY_EXISTS} -> error
  end.
