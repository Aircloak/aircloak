...
Furthermore, the aggregators `min`, `max`, and `median` cannot be used in anonymizing queries and subqueries (see
[Query and subquery types](/sql.md#query-and-subquery-types) for more about this distinction). This is due to the mode
of operation of these aggregators and the fact that they require estimating the spread in subsets of the data.  See
[Aggregates](/sql/query-results.md#anonymising-aggregation-functions) for more on this topic.
