...
By default, when the air site is accessed from the browser, a websocket connection is established. This connection is used to push real-time notifications in various situations. If the air is behind a proxy, and you don't want to allow forwarding of websocket connections, you can explicitly force long polling protocol. This can be done by setting the `browser_long_polling` option to `true`.
