defmodule Central.Service.Customer do
  ...
  def import_customer_data(air_export) do
    with {:ok, export} <- AirMessage.decode_exported_data(air_export),
         ...
         {:ok, handler} <- air_handler(export.air_version) do
      {:ok, _} =
        Repo.transaction(
          fn ->
            try do
  ...
  def imported?(customer, export_id),
    do:
      Repo.one(
        from(
          c in CustomerExport,
          where: c.export_id == ^export_id and c.customer_id == ^customer.id
        )
      ) != nil

  @doc "Returns the last imported export for the given customer."
  @spec most_recent_export(Customer.t()) :: CustomerExport.t() | nil
  def most_recent_export(customer),
    do:
      Repo.one(
        from(
          c in CustomerExport,
  ...
  defp mark_export_as_imported!(customer, export_id, created_at),
    do:
      Repo.insert!(%CustomerExport{
        export_id: export_id,
  ...
  @doc false
  def child_spec(_arg), do: Aircloak.ChildSpec.task_supervisor(name: @message_handler_sup)
end
