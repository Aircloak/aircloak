defmodule Central.Service.Customer.Message do
  ...
  @spec import_customer_data(binary) :: {:ok, pos_integer} | {:error, import_error}
  def import_customer_data(air_export) do
    with {:ok, export} <- decode_exported_data(air_export),
         {:ok, customer} <- Customer.from_token(export.customer_token),
  ...
  defp validate_export(customer, export) do
    cond do
