defmodule Central.CustomerMessage do
  ...
  def import_customer_data(air_export) do
    with {:ok, data} <- decode_exported_data(air_export),
         {:ok, customer} <- Customer.from_token(data.customer_token) do
      Enum.each(data.rpcs, &handle(&1, customer, data.air_name))
      {:ok, length(data.rpcs)}
    end
  end
  ...
  defp decode_exported_data(air_export) do
    try do
      %{"air_name" => air_name, "customer_token" => customer_token, "rpcs" => rpcs} =
        air_export
        |> :zlib.gunzip()
        |> Poison.decode!()

      {:ok, %{air_name: air_name, customer_token: customer_token, rpcs: rpcs}}
    catch type, error ->
      Logger.error([
