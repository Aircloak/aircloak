...
```{r}
renderTable({
  d <- dataForVisualization(input$customer, queryResults$dataSources, queryResults$dataSourcesByCustomer)
  d$driver <- sapply(strsplit(d$driver, "\\."), tail, n=1)
  d
})
```
...
renderPlot({
  ...
  ggplot(data=dataset, aes(x=executionTime, y=count)) +
    geom_col() +
    xlab("Query execution time /s") + ylab("Number of queries")
})
```
