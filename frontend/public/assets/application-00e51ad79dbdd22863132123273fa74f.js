...
(function() {
  var activate_fileupload, filterOutScheduledForRemoval;

  this.app = angular.module("queryEditor", []);

  filterOutScheduledForRemoval = function(items) {
    return _.filter(items, function(item) {
      return !!!item.scheduleRemove;
    });
  };

  this.app.filter("unlessScheduledForRemoval", function() {
    return function(items) {
      return filterOutScheduledForRemoval(items);
    };
  });

  this.app.directive("indices", function() {
    return {
      restrict: "E",
      ...
      link: function(scope) {
        scope.addIndex = function() {
          ...
          return scope.selectedIndex = void 0;
        };
        scope.removeIndex = function(i) {
          ...
          return scope.queryFile.indices = _.without(scope.queryFile.indices, i);
        };
        scope.notInIndicesForQueryFile = function(i) {
          return _.filter(i, function(e) {
            return _.indexOf(scope.queryFile.indices, e) === -1;
          });
        };
        scope.newIndex = function(event) {
          var i;
          i = {
            name: scope.name,
          ...
          return scope.availableIndices.push(i);
        };
        return scope.addIndexToQueryFile = function(i) {
          if (scope.queryFile.indices === void 0) {
            scope.queryFile.indices = [];
          }
          return scope.queryFile.indices.push(i);
        };
      }
    };
  });

  this.QueryEditorCtrl = function($scope, $window) {
    var _this = this;
    $scope.data = {};
    $scope.data.query_files = [];
    $scope.temp = void 0;
    $window.addQueryFile = function(result) {
      result.fresh = true;
      $scope.temp = result;
      return $scope.$apply("appendQueryFile()");
    };
    $scope.appendQueryFile = function() {
      $scope.data.query_files.push($scope.temp);
      return $scope.temp = void 0;
    };
    $scope.numberOfMainClasses = function() {
      var f;
      f = function(count, q) {
      ...
      return _.reduce(filterOutScheduledForRemoval($scope.data.query_files), f, 0);
    };
    $scope.tooManyMainClasses = function() {
      return $scope.numberOfMainClasses() > 1;
    };
    return $scope.removeQueryFile = function(queryFile) {
      queryFile.scheduleRemove = true;
      return console.log(queryFile);
    };
  };

  activate_fileupload = function() {
    return $("#query_files_form").fileupload({
      dataType: "json",
      add: function(e, data) {
        var file, types;
        ...
        if (types.test(file.type) || types.test(file.name)) {
          ...
          return alert("" + file.name + " is not a class file");
        }
      },
      ...
      fail: function(e, data) {
        return console.log("Upload failed: e: " + e + ", data: " + data);
      },
